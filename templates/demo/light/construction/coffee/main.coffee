...
$ ->
	...
	# show content after $(window).load
	$window.load( ->
	...
	if ( $("a[data-gal^='prettyPhoto']").length and $.fn.prettyPhoto )
		$("a[data-gal^='prettyPhoto']").prettyPhoto({hook: 'data-gal'}) # use 'data-gal' instead of 'rel' to pass HTML5 validator
	...
	if ( $('audio').length )
		...
		$('.mejs-audio').css('display', 'inline-block') # for layout
