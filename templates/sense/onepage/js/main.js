   ...
   if ( $("#nav").length ) {
		...
		if( $.fn.smoothScroll ) {
			nav.find('a').smoothScroll( {
				offset: -80 //Since there's a top-fixed-nav
			} );		
		}

		// change the CSS when clicked
		nav.children('li').on('click', function(){
			$(this).siblings('li').removeClass('active').end()
				.addClass('active'); //why add first, remove siblings second doesn't work?
		});

		// off canvas Nav
		$('a.menu').click(function() {
			if ( $(this).parents('header').siblings('nav').hasClass('open') ){
				$(this).parents('header').siblings('nav').removeClass('open');
			}
			else {
				$(this).parents('header').siblings('nav').addClass('open');
			}
			return false;
		});   	
	}
